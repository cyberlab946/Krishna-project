// =========================================
// CYBERLAB PROFILE - DASHBOARD PROFILE LINK + EDIT NAME
// =========================================
...
function showEditProfile() {
    ...
    box.innerHTML = `<button class="close" onclick="closeLogin()" aria-label="Close profile editor">×</button><h2>Edit <span>Profile</span></h2><p style="margin-bottom:15px;">Change the name shown on your CyberLab profile.</p><input id="editProfileName" type="text" value="${safeName}" placeholder="Your Full Name" autocomplete="name"><button id="saveProfileButton" onclick="saveProfileName()">Save Profile</button><p><a href="#" onclick="closeLogin(); return false;">Cancel</a></p>`;
...
async function saveProfileName() {
    ...
    if (name.length < 2) return alert('Name must contain at least 2 characters.');
    if (button) { button.disabled = true; button.textContent = 'Saving...'; }
    try {
        var result = await supabaseClient.auth.updateUser({data:{full_name:name}});
        if (result.error) return alert('Could not update profile: ' + result.error.message);
...
function updateDashboardUser() {
    ...
    var fullProfileButton = document.getElementById('fullProfileButton');
    if (!fullProfileButton && profileElement) {
        fullProfileButton = document.createElement('a');
        fullProfileButton.id = 'fullProfileButton';
        fullProfileButton.href = 'profile.html';
        fullProfileButton.textContent = 'View Full Profile';
        fullProfileButton.style.cssText = 'display:inline-block;margin:12px 0 0 8px;padding:8px 14px;border:1px solid #29445e;border-radius:5px;color:#00ff9d;text-decoration:none;font-weight:bold;';
        profileElement.appendChild(fullProfileButton);
    }
    ...
    if (currentUser) {
        ...
        if (fullProfileButton) fullProfileButton.style.display = 'inline-block';
    } else {
        ...
        if (fullProfileButton) fullProfileButton.style.display = 'none';
    }
}
...
window.addEventListener('keydown', function(event) {
    if (event.key === 'Enter') {
        ...
        if (modal && modal.classList.contains('active') && active && active.id === 'editProfileName') saveProfileName();
    }
});
